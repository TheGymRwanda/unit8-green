...
function GallerySection() {
    ...
    const createImages= async(event)=>{
      const mouseMoveDistance = ()=>Math.hypot(event.movementX, event.movementY)
      if(mouseMoveDistance()>10||event.type==='click'){
        if(imageNum.current===IMAGES.length){
          imageNum.current =0;
        }
     const image = hiddenImg.current.querySelectorAll('img')[imageNum.current++]
    ...
    await prevContext.current.fillRect(posx,posy, prevImage.current.width, prevImage.current.height)
  }
  ...
  return (
    <Wrapper>
      <div ref={hiddenImg} className='hidden'>
        {IMAGES.map((image,index)=><img key={index} src={image.image}/>)}
      </div>
      <div className='' id='canva'>
        <canvas  ref={canvasRef} className='h-[856px] max-h-screen mb-[111px] scale-100 w-full mx-auto border-2 ' onClick={createImages} onMouseMove={createImages} onLoad={createImages} >
